...
class Hospital:
    ...
    def buscar_medico_nombre(self, nombre):
        return next(
            (m for m in self.medicos if m.nombre == nombre), None
        )

    def buscar_medicos_por_especialidad(self, especialidad):
        return [m.nombre for m in self.medicos if m.especialidad == especialidad]
            
    def borrar_paciente(self, paciente):
        if paciente in self.pacientes:
            self.pacientes.remove(paciente)

    def borrar_medico(self, medico):
            ...
            with open(archivo, newline="", encoding="utf-8") as csvfile:
                ...
                for row in reader:
                    ...
                    if paciente and medico:
                        ...
                        cita_urgente = row.get("Cita_urgente", "No")
                        cita = Cita(paciente, medico, fecha_hora, cita_urgente)
